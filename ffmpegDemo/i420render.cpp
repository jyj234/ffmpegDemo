...
{
    if (!mProgram.addShaderFromSourceFile(QOpenGLShader::Vertex, "D:/code/FFmpeg_Git/release/Shaders/vertex.vsh"))
    {
        qWarning() << " add vertex shader file failed.";
        return;
    }
    if (!mProgram.addShaderFromSourceFile(QOpenGLShader::Fragment, "D:/code/FFmpeg_Git/release/Shaders/fragment.fsh"))
    {
...
void I420Render::updateTextureInfo(int width, int height, int format)
{
    ...
    {
        qDebug()<<"start updateTextureInfo";
        // yuv420p
...
{
    // qDebug()<<"start updateTextureData";
    ...
    // qDebug()<<"after updateTextureData";
}
